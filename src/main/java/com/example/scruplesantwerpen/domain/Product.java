    ...
    public void generateAndSetEAN13BarcodeImage() throws Exception {
        if(barcode != null) {
            ...
            if (lengte < 12) {
                ...
                /*Als de barcode gemaakt wordt, moet de image ervan naar de juiste directory*/
                File file = new File("/home/muski/IdeaProjects/scruplesAntwerpenTwee-master/src/main/resources/static/images" + langeId);
                BarcodeImageHandler.saveJPEG(barcode, file);

        }
    }

    public String getBarcodeSrcId() {
        if(barcode != null) {
            var lengte = String.valueOf(getProductId()).length();
            StringBuilder langeId = new StringBuilder(String.valueOf(productId));
            if (lengte < 12) {
